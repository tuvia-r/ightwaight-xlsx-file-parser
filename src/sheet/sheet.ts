...
export class Sheet {
    ...
    get rowsAsJson () {
        const res: any[] = []
        for(let i = this.startRow; i < this.numberOfRows; i++){
    ...
    getRow(index: number) {
        if(index > this.numberOfRows || index < this.startRow){
            throw new Error(`index ${index} out of bounds`)
        }
        const numberOfColumns = this.numberOfColumns;
        const rowCells: Cell[] = []
        for (let col = this.startColumn; col < numberOfColumns; col++){
    ...
    getColumn(index: number) {
        if(index > this.numberOfColumns || index < this.startColumn){
            throw new Error(`index ${index} out of bounds`)
        }
        const numberOfRows = this.numberOfRows;
        const colCells: Cell[] = []
        for (let row = this.startRow; row < numberOfRows; row++){
    ...
    getCell(row: number, column: number) {
        let cell = this.cells.find(c => c.row === row && c.column === column);
        if(!cell){
            cell = new Cell({
                column,
                row,
